...
public class CraftingTransferHandler<C extends RecipeBookMenu<?>> implements IRecipeTransferHandler<C, CraftingRecipe> {
    ...
    private final IRecipeTransferHandler<C, CraftingRecipe> fallback;
    ...
    public CraftingTransferHandler(IRecipeTransferHandlerHelper helper,
                                   IRecipeTransferHandler<C, CraftingRecipe> fallback,
    ...
    @Override
    @Nullable
    public IRecipeTransferError transferRecipe(C menu, CraftingRecipe recipe, IRecipeSlotsView slots,
        ...
        if (doTransfer) {
            LINet.toServer(new RecipeTransferPacket(recipe.getId(), maxTransfer));
        }
        return null;
    }
    ...
    private static boolean claim(IRecipeSlotView view, ItemStack[] stacks, int[] counts) {
        ...
        for (ItemStack option : options) {
            for (int i = 0; i < stacks.length; i++) {
                if (counts[i] > 0 && ItemStack.isSameItemSameTags(stacks[i], option)) {
                    counts[i]--;
                    return true;
                }
            }
