...
public class CombinedInventoryHandler implements IItemHandlerModifiable {
    ...
    public @NotNull ItemStack insertItem(int slot, @NotNull ItemStack stack, boolean simulate) {
        ...
        if (!simulate) {
            if (existing.isEmpty()) {
                inventory.setItem(MAIN_OFFSET + slot, ItemHandlerHelper.copyStackWithSize(stack, moved));
            } else {
                existing.grow(moved);
            }
            inventory.setChanged();
        ...
        return overflow ? ItemHandlerHelper.copyStackWithSize(stack, stack.getCount() - moved) : ItemStack.EMPTY;
    }
    ...
    public @NotNull ItemStack extractItem(int slot, int amount, boolean simulate) {
        ...
        if (!simulate) {
            inventory.setItem(MAIN_OFFSET + slot, ItemHandlerHelper.copyStackWithSize(existing, existing.getCount() - taken));
            inventory.setChanged();
        }
        return ItemHandlerHelper.copyStackWithSize(existing, taken);
    }
