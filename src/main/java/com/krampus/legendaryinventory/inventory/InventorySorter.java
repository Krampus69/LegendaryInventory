...
public final class InventorySorter {
    ...
    private static int foodCompare(ItemStack a, ItemStack b) {
        FoodProperties fa = a.get(DataComponents.FOOD);
        FoodProperties fb = b.get(DataComponents.FOOD);
        if (fa == null || fb == null) {
            return 0;
        }
        int byNutrition = fb.nutrition() - fa.nutrition();
        if (byNutrition != 0) {
            return byNutrition;
        }
        return Float.compare(fb.saturation(), fa.saturation());
    }

    private static int tierCompare(ItemStack a, ItemStack b) {
        int ta = a.getItem() instanceof TieredItem t ? t.getTier().getUses() : 0;
        int tb = b.getItem() instanceof TieredItem t ? t.getTier().getUses() : 0;
        return tb - ta;
    }

    private static int swordCompare(ItemStack a, ItemStack b) {
        float da = a.getItem() instanceof SwordItem s ? s.getTier().getAttackDamageBonus() : 0.0F;
        float db = b.getItem() instanceof SwordItem s ? s.getTier().getAttackDamageBonus() : 0.0F;
    ...
    private enum Category {
        ...
        FOOD(stack -> stack.has(DataComponents.FOOD), InventorySorter::foodCompare),
